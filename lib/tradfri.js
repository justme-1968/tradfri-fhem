...
async function start() {
  ...
  if( this.security_code && !this.credentials ) {
    ...
    this.credentials.psk = psk;
  }

  //console.log( '*** FHEM: identity: '+ this.credentials.identity );
  //console.log( '*** FHEM: psk: '+ this.credentials.psk );
...
Tradfri.prototype.device2HUE = function( device ) {
  ...
  if( device.type === AccessoryTypes.lightbulb ) {
    ...
    hue.r = 'sensor';
    hue.state = { lastupdated: new Date(device.lastSeen*1000).toISOString() };
