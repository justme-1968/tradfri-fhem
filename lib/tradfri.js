 ...
 async function operate(json) {
  ...
  if( json.t === 'group' ) {
    ...
    if( group ) {
      ...
      if( json.stop !== undefined && json.stop )
        group.stopBlinds();
      else if( json.pct !== undefined )
        group.setPosition( json.pct );
      else
...
Tradfri.prototype.device2HUE = function( device ) {
  ...
  } else if( device.type === AccessoryTypes.remote || device.type === AccessoryTypes.slaveRemote || device.type === AccessoryTypes.soundRemote ) {
    hue.r = 'sensor';
    hue.config = { battery: device.deviceInfo.battery };
  ...
  } else if( device.type === AccessoryTypes.signalRepeater ) { // repeater
    hue.r = 'sensor';
    hue.config = { battery: device.deviceInfo.battery };
    hue.state = { lastupdated: new Date(device.lastSeen*1000).toISOString().replace(/\..+/, '') };
  }
